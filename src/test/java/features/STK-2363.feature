Feature: Verifying empty states for products area
  ...
  @STK-2363/2472
    #[STK-2363/2472] Empty state - Products area - Stacks
      #run test with username=lily.king@stackedinvest.com
  ...
  Scenario: A user with no Stacks configured will see the Stacks tab as an empty state
    ...
    When a user clicks on any stack card, the stack's details page is opened, "Stack Composition" displays
    When a user is on the dashboard page and clicks on the Explore Stacks button
    Then the stacks tab of the market place is opened and the "Diversify your crypto into pre-built Stacks." text displays
  ...
  @STK-2363/2475 #inprogress #need user on new dashboard with no bots to test with new dashboard access
